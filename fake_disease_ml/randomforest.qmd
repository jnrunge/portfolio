...
Let's have a look at the traits we have.
...
ggplot(data_combined, aes(age, fill = phenotype)) +
  geom_histogram(position = "fill")+theme_bw(16)
...
So, concluding for now, we have built a random forest that is a able find \> 90% of individuals with an imaginary disease that decreases their physical fitness and is determined by three loci in a additive and epistatic fashion. We would probably not be able to use this model on a large number of loci because of performance, but I am curious how well it still works when there are more loci (of course, building a new forest with those first).
